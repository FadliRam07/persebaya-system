...
const HERO_SLIDES = [
  { 
    ...
    subtitle: "Performa semakin meningkat, Green Force siap mengincar kemenangan demi kemenangan.", 
...
export default function Home({ newsList, onGoToNews, onGoToPlayers, onGoToSchedule, isDarkMode }) {
  ...
  const openChatbot = () => {
    ...
    if (chatbotToggle) {
      ...
      // Try to find chatbot button by text or class
    ...
    // Scroll to bottom where chatbot is located
    setTimeout(() => {
  ...
  return (
    <>
      {/* ✅ WELCOME CARD - NOW AT THE TOP (More Compact) */}
      <div className="mb-6 sm:mb-8">
        <div className={`${isDarkMode ? 'bg-gradient-to-br from-persebaya-green to-emerald-800' : 'bg-gradient-to-br from-persebaya-green to-emerald-700'} rounded-2xl shadow-2xl overflow-hidden`}>
          <div className="p-6 sm:p-8 text-white">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h2 className="text-2xl sm:text-3xl font-bold mb-2">Selamat Datang di Website Persebaya Surabaya</h2>
                <p className="text-white/90 text-sm sm:text-base mb-4 leading-relaxed">
                  Rumah digital untuk seluruh Bonek. Temukan berita, jadwal, dan info lengkap klub kebanggaan Arek-arek Suroboyo.
                </p>
                
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  <button 
                    onClick={onGoToSchedule}
                    className="bg-white text-persebaya-green px-4 py-2 rounded-lg font-bold text-sm hover:bg-gray-100 transition shadow"
                  >
                    Jadwal
                  </button>
                  <button 
                    onClick={onGoToPlayers}
                    className="bg-white/20 backdrop-blur-sm text-white border border-white/30 px-4 py-2 rounded-lg font-bold text-sm hover:bg-white/30 transition"
                  >
                    Pemain
                  </button>
                  <button 
                    onClick={openChatbot}
                    className="bg-persebaya-dark text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-gray-800 transition shadow flex items-center gap-1"
                  >
                    Tanya AI
                  </button>
                </div>
      ...
      <div className="relative h-48 sm:h-64 md:h-80 lg:h-96 rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl mb-6 sm:mb-8">
